...
- (instancetype)initWithNode:(_QTEventBusLinkNode *)node {
    if (self = [super init]) {
        ...
        _registeredNodeTable[node.uniqueId] = node;
    }
    return self;
}
...
- (void)removeNodeForId:(NSString *)uniqueId {
    //不存在
    if (!_registeredNodeTable[uniqueId]) {
        return;
    }
    _QTEventBusLinkNode *node = _registeredNodeTable[uniqueId];
    if (node == _head) {
        _head = _head.next;
    }
    if (node == _tail) {
...
- (void)appendNode:(_QTEventBusLinkNode *)node {
    ...
    _QTEventBusLinkNode *oldNode = _registeredNodeTable[node.uniqueId];
    if (oldNode) {
...
- (void)replaceNode:(_QTEventBusLinkNode *)old withNode:(_QTEventBusLinkNode *)update {
    ...
    _registeredNodeTable[update.uniqueId] = update;
}
...
@interface VVEventBusCollection () {
...
@implementation VVEventBusCollection
